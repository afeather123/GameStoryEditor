  ...
  private handleHighlight(result, options: AngularFusejsOptions) {
    ...
    return result.map((matchObject) => {
      ...
      for (const match of matchObject.matches) {
        ...
        if (_.get(item[options.fusejsHighlightKey], key).constructor === Array) {
          key += `[${match.arrayIndex}]`;
        }

        for (const indice of indices) {
          const initialValue: string = _.get(item[options.fusejsHighlightKey], key) as string;
          ...
          _.set(item[options.fusejsHighlightKey], key, newValue);
        }
      }
